...
| 命令 | 用途 |
|------|------|
| `python checker.py file.cpp` | 单文件检查 |
| `python checker.py a.cpp b.cpp` | 双文件对比 |
| `python checker.py src/` | 目录批量检查 |
| `python checker.py dir1/ dir2/` | 两个文件夹对比 |

## Workflow
1. **Setup（首次）**: `python setup.py` 下载 cppcheck + clang-tidy 到 `download/` 目录。若 `download/` 下已存在对应工具则跳过下载。
2. **Check**: `python checker.py <file(s) or dir(s)>` 运行分析。**运行前须确保至少一种工具已安装**；若 `download/` 下无任何工具，须先执行 `setup.py`。
